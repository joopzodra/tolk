  ...
  select(term: string, column: string, sheet: string, byBegin: boolean) {
    ...
    const maybeByBegin = (dbTable: Dexie.Table<SheetRow, number>) => {
      if (byBegin) {
        ...
        const escapeRegExpTerm = term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // $& means the whole matched string
        const regex = new RegExp(escapeRegExpTerm, 'i');
        return dbTable.filter(words => regex.test(words[column]));
      }
    }
